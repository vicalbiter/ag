...
public class GAUtils {
    ...
    ****************************************************/
    ...
    public Individual generateNNIndividual(int n, double min, double max) {
        String ind = "";
        for (int i = 0; i < n; i++) {
            float r = generateRandomFloatNumber(min, max);
            ind += floatToBinaryString(r);
        }
        return new Individual(ind);
    }
    ...
    ****************************************************/
    ...
    public float binaryStringToFloat(String str) {
        ...
        String right = str.substring(4, 32);
        float intsign = 0;
        int intpart = 0;
        double decpart = 0;
        if (sign == '0') { intsign = 1; }
        else if (sign == '1') { intsign = -1; }
        intpart = Integer.parseInt(left, 2);
        decpart = Integer.parseInt(right, 2) / (Math.pow(2, 28) - 1);
        float floatnumber = intsign * (float) (intpart + decpart); 
        return floatnumber;
    }
    
    // Convert a floating point number to its 32-binary string representation
    public String floatToBinaryString(float flo) {
        String stringfloat = String.format("%.28f", flo);
        String binary = "";
        String nosign = "";
        if (stringfloat.charAt(0) == '-') {
            nosign = stringfloat.substring(1);
            binary = "1";
        }
        else {
            nosign = stringfloat;
        ...
        int intpart = Integer.parseInt(nosign.substring(0,1));
        int decpart = (int) (Double.valueOf(nosign.substring(1)) * (Math.pow(2, 28) - 1));
        String left = String.format("%3s", Integer.toBinaryString(intpart)).replace(" ", "0");
        String right = String.format("%28s", Integer.toBinaryString(decpart)).replace(" ", "0");
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        /*
        Individual ind = utils.generateIndividual(32);
        System.out.println(ind);
        float num = utils.binaryStringToFloat(ind.toString());
        System.out.println(num);
        System.out.println(utils.floatToBinaryString(num));
        /*
        
        /*
        Population population = utils.generatePopulation(2, 32);
        for (Individual s : population.getPopulation()) {
            System.out.println(s + " " + utils.binaryStringToFloat(s.toString()));
        }
        */
