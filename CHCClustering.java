...
public class CHCClustering {

    private Individual best_individual;
    private double best_fitness;
    
    // Class constructor
    public CHCClustering(int n, int a, int g, double[][] input, int cat) {
        ...
        Population population = utils.generateIntegerPopulation(n, a, input.length);
        double[] fitness = utils.getFitnessOfIntegerPopulation(population, input);
        ...
        double temp_fitness = utils.getFitnessOfIntegerIndividual(temp_individual, input);
        ...
        for (int i = 0; i < n; i++) {
            ...
            double current_fitness = utils.getFitnessOfIntegerIndividual(current_individual, input);
        ...
        for (int i = 0; i < g; i++) {
            ...
            for (int j = 0; j < n/2; j++) {
                ...
                new_one.setAlleles(a);
                new_one_fitness = utils.getFitnessOfIntegerIndividual(new_one, input);
                new_two = offspring_buffer[1];
                new_two.setAlleles(a);
                new_two_fitness = utils.getFitnessOfIntegerIndividual(new_two, input);
            ...
            fitness = utils.getFitnessOfIntegerPopulation(population, input);
            ...
            temp_fitness = utils.getFitnessOfIntegerIndividual(temp_individual, input);
            current_best = new CIndividual(temp_individual, temp_fitness);
            System.out.println(current_best.fitness);
            if (current_best.fitness > previous_best.fitness) {
                ...
                if (cataclism_counter >= cat) {
                    ...
                    for (int j = 1; j < population.length(); j++) {
                        Individual current_individual = current_best.ind.mutateInteger(0.35);
                        double current_fitness = utils.getFitnessOfIntegerIndividual(current_individual, input);
        ...
        System.out.println("Fitness: " + utils.getFitnessOfIntegerIndividual(best, input));
        Wines wines = new Wines(input, best.toString(), best.getAlleles());
        System.out.println("VNND: " + wines.vnnd());
        
        this.best_individual = best;
        this.best_fitness = utils.getFitnessOfIntegerIndividual(best, input);
    }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        CHCClustering chc = new CHCClustering(70, 3, 500, input, 20);
    }
}